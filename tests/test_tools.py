...
def test_discover_source_files():
    ...
    files, truncated, _blob_shas = discover_source_files(tree_entries, gitignore_content=None)
...
def test_discover_source_files_respects_max():
    ...
    files, truncated, _blob_shas = discover_source_files(tree_entries, max_files=100)
...
def test_discover_source_files_explicit_max_overrides_env():
    ...
    with patch.dict("os.environ", {MAX_INDEX_FILES_ENV_VAR: "7"}, clear=False):
        files, truncated, _blob_shas = discover_source_files(tree_entries, max_files=5)
